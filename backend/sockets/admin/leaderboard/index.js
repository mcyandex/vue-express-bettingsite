...
} = require('../../../controllers/admin/leaderboard');
    ...
    socket.on('getLeaderboardList', async(data, callback) => {
        ...
        try {
            ...
            try {
                ...
                adminGetLeaderboardListSocket(io, socket, user, data, callback);
            } catch(err) {
        ...
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendLeaderboardCreateSocket(io, socket, user, data, callback);
                } catch(err) {
        ...
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendLeaderboardStopSocket(io, socket, user, data, callback);
                } catch(err) {
                    socketRemoveAntiSpam(socket.decoded._id);
                    callback({ success: false, error: { type: 'error', message: err.message } });
                }
