...
} = require('../../../controllers/admin/rain');

module.exports = (io, socket) => {

    socket.on('getRainList', async(data, callback) => {
        ...
        try {
            ...
            try {
                ...
                adminGetRainListSocket(io, socket, user, data, callback);
            } catch(err) {
    ...
    socket.on('sendRainAmount', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendRainAmountSocket(io, socket, user, data, callback);
                } catch(err) {
