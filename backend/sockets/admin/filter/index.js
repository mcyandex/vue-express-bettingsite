...
} = require('../../../controllers/admin/filter');

module.exports = (io, socket) => {

    socket.on('getFilterList', async(data, callback) => {
        ...
        try {
            ...
            try {
                ...
                adminGetFilterListSocket(io, socket, user, data, callback);
            } catch(err) {
    ...
    socket.on('sendFilterCreate', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendFilterCreateSocket(io, socket, user, data, callback);
                } catch(err) {
    ...
    socket.on('sendFilterRemove', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendFilterRemoveSocket(io, socket, user, data, callback);
                } catch(err) {
                    socketRemoveAntiSpam(socket.decoded._id);
                    callback({ success: false, error: { type: 'error', message: err.message } });
                }
