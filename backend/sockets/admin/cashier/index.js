...
} = require('../../../controllers/admin/cashier');

module.exports = (io, socket) => {

    socket.on('getCashierList', async(data, callback) => {
        ...
        try {
            ...
            try {
                ...
                adminGetCashierListSocket(io, socket, user, data, callback);
            } catch(err) {
    ...
    socket.on('sendCashierCryptoAction', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendCashierCryptoActionSocket(io, socket, user, data, callback);
                } catch(err) {
    ...
    socket.on('sendCashierRobuxCancel', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendCashierRobuxCancelSocket(io, socket, user, data, callback);
                } catch(err) {
                    socketRemoveAntiSpam(socket.decoded._id);
                    callback({ success: false, error: { type: 'error', message: err.message } });
                }
    ...
    socket.on('sendCashierCreate', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminSendCashierCreateSocket(io, socket, user, data, callback);
                } catch(err) {
                    socketRemoveAntiSpam(socket.decoded._id);
                    callback({ success: false, error: { type: 'error', message: err.message } });
                }
